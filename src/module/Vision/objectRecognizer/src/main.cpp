...
private:
    ...
    void onRead(Image &img)
    {
        ...
        if (img.width()>0 && img.height()>0)
        {
            ...
            label_in_object.read(inLabel);
            std::string input_label = inLabel.get(0).asString();
            ...
            if (crop_found)
            {
                ...
                if (crop_valid)
                {
                    ...
                    if (port_out_view.getOutputCount())
                    {
                        ...
                        std::string max_label = labels[max_idx].c_str();
                        ...
                        Bottle match_b;
                        if (input_label == max_label){
                            match_b.addInt(1);
                        }
                        else{
                            match_b.addInt(0);
                        }
                        match_object.write(match_b);
...
public:
    ...
    ObjectRecognizerPort(ResourceFinder &rf) : BufferedPort<Image>()
    {
        ...
        caffe_wrapper = new CaffeWrapper<float>(caffemodel_file, prototxt_file,
            ...
            label_in_object.open(("/"+name+"/in_label:i").c_str());
            ...
            match_object.open(("/"+name+"/match:o").c_str());
